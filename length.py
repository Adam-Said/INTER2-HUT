...
def main(corpus):
    ...
    fd = open("rapport_longueur.csv", "w", encoding="utf-8", errors="surrogateescape")
    fd.write("sep=;\nAdresse;Longueur moyenne;Nombre de mails\n")
    for adresse in tabAdresses:
        print(adresse + " a écrit en moyenne " + str(tabAdresses[adresse][1]) + " lignes en " + str(tabAdresses[adresse][0]) + " mail(s)\n")
        fd.write(adresse + ";" + str(tabAdresses[adresse][1]) + ";" + str(tabAdresses[adresse][0]) + "\n")
    fd.close()
